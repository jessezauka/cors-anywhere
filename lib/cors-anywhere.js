...
function proxyRequest(req, res, proxy) {
  ...
  req.on('close', () => {
    proxyReq.destroy(); // Stop the upstream request
  ...
  try {
    ...
    // Send heartbeat data every 25 seconds
    const heartbeatInterval = setInterval(() => {
      res.write(' '); // Send a blank space
    }, 25000);

    // Stop the heartbeat when the client disconnects
    req.on('close', () => {
